            ...
            <View style={styles.topInfo}>
              ...
              <View style={[styles.statusBadge, isLive && styles.statusBadgeLive]}>
                <Text style={[styles.statusText, isLive && styles.statusTextLive]}>
                  {isLive ? `🔴 ${status === 'HT' ? 'Mi-temps' : status + "'"}` : status === 'FT' ? '✅ Terminé' : `⏰ ${status}`}
                </Text>
              </View>
            </View>
                    ...
                    {detail.stats && (
                      <View style={styles.statsCard}>
                        ...
                        {detail.stats.total_localteam_scored !== undefined && (
                          <StatRow
                            label="Buts domicile"
                            value={detail.stats.total_localteam_scored}
                            color="#FFD700"
                          />
                        )}
                        {detail.stats.total_visitorteam_scored !== undefined && (
                          <StatRow
                            ...
                            value={detail.stats.total_visitorteam_scored}
                            color="#FF6B6B"
                          />
                        )}
                      </View>
                    ...
                    {detail.h2hMatches && detail.h2hMatches.length > 0 ? (
                      <>
                        ...
                        {detail.h2hMatches.map((h: any, i: number) => {
                          const parts = h.scoretime?.split('-').map((s: string) => s.trim()) || ['-', '-'];
                          const homeWon = parseInt(parts[0], 10) > parseInt(parts[1], 10);
                          const awayWon = parseInt(parts[1], 10) > parseInt(parts[0], 10);
...
const styles = StyleSheet.create({
  ...
  statusBadgeLive: { backgroundColor: '#39FF1420' },
  statusText: { color: '#888', fontSize: 11, fontWeight: 'bold' },
